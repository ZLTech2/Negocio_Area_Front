...
const Footer = () => {
  ...
  const { fotoPerfilEmpresa, visitante, authTipo } = useContext(UserContext);
  const fotoPerfil = fotoPerfilEmpresa;
  ...
  return (
    <View style={styles.rodape}>
      ...
      {/* Botões do footer */}
      <Pressable
        onPress={() => navigation.navigate('FeedEmpresa', { irParaHome: true })}
        style={({ pressed }) => [
      ...
      <Pressable
        onPress={() => navigation.navigate('TelaBuscar', { abrirBusca: true })}
        style={({ pressed }) => [
          pressed && { transform: [{ scale: 1.5 }], opacity: 0.4 },
        ]}>
      ...
      <Pressable
        onPress={() => navigateIfAllowed('TelaLocalizacao')}
        style={({ pressed }) => [
          pressed && { transform: [{ scale: 1.5 }], opacity: 0.4 },
        ]}>
      ...
      <Pressable
        onPress={() => navigateIfAllowed(authTipo === 'cliente' ? 'TelaPerfilCliente' : 'TelaPerfilEmpresa')}
      >
